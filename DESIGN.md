...
8. **空态断言必须区分「没有选中」与「一个也没有」**：判据是列表长度（如 `ontologyList.length` / `projects.length`），不是「当前有没有选中项」。账号里已有资产却写「尚未创建本体」「创建第一个本体」「还没有项目」是自相矛盾的空态——同一屏下方往往就列着可「打开」的已有条目，且与选择器占位文案（「未选择本体」）口径打架。`App.vue:58` 那条注释治的是加载竞态闪一帧，本条治的是状态语义本身：文案分支按列表长度取，两种情形各自的动作指引要不同（未选→去列表里选一个，真空→去新建）。同理 `document.title` 不得在应用名两侧都填占位值（未选本体时不得拼成「本体工作台 · 本体工作台」）。
