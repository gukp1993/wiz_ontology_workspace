...
修复语义（逐条对应验收要求）：
- 对**被项目引用**的编排调用 flows.check_flow(flow_state)（纯配置检查：不传
...
FLOW_OK = make_flow('合法编排', wired=True,
                    outputs=[{'name': 'power', 'label': '功率', 'type': {'type': 'number'}}])
# 编排 W：仅有 warning 无 error（SQL 节点引用的连接待项目侧确认）→ 不阻断
FLOW_WARN = make_flow('仅警告编排', wired=False,
                      outputs=[{'name': 'power', 'label': '功率', 'type': {'type': 'number'}}])
_state = flows.read_draft(FLOW_WARN)
_state.pop('_draft')
_state['nodes'] = [{'id': 'nd_sql', 'kind': 'sql', 'name': '取数', 'inputs': [],
                    'outputs': [{'id': 'nd_out_power', 'name': 'power', 'label': '功率',
                                 'type': {'type': 'number'}}],
                    'implementation': {'connectionId': 'ghost_conn', 'sql': 'SELECT 1'}}]
...
def _counting_check_flow(state, **kw):
    _calls.append(state.get('flowId') if isinstance(state, dict) else None)
    return _real_check_flow(state, **kw)
