"""Q02 深度测试公共 HTTP 客户端（只读业务代码，仅测试用）。

约定：
- 目标服务为已运行的隔离实例 http://127.0.0.1:18931（不启动/不停止）。
- 所有请求走真实 HTTP 生产入口；本模块只负责加 Cookie、Origin 与解析响应。
- POST 必须带 Origin（server.py 白名单校验）。
- 证据以 JSONL 追加写入 .runtime/test-evidence/q02/。
"""
import json
import os
import sys
...
def as_schema(state):
    """把 legacy @graph 形态转成 schema 形态（服务端 encode 等价实现，走后端库函数确保口径一致）。"""
