...
<ObjectWorkspace v-if="view==='objects'" :state="state" :focus-type="propertyFocusType" :focus-property="propertyFocusId" :initial-tab="objectDetailTab" :focus-definition="definitionFocusId" :focus-create="objectCreateFocus" :graph-return="objectGraphReturn" :graph-focus="objectGraphFocus" :canvas-return="canvasReturn?.nodeId || ''" :save-state="saveState" :latest-release="latestVersion" @before-change="pushUndo" @changed="changed" @navigate="navigate" @switch-ontology="id => switchOntology(id)" @create-ontology="name => createOntologyNamed(name)"/>
