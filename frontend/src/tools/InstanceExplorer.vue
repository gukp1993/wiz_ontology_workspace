...
function persist(){try{prefSet(storageKey,JSON.stringify(saved.value));notice.value='已保存到当前浏览器。'}catch{notice.value='浏览器存储不可用，探索配置仅在当前页面保留。'}}
...
onMounted(()=>{try{const parsed=JSON.parse(prefGet(storageKey)||'[]');if(Array.isArray(parsed))saved.value=parsed.filter(e=>typeof e.name==='string'&&e.filter&&['query','type','property','operator','value'].every(k=>typeof e.filter[k]==='string')&&modes.some(m=>m.value===e.view)).slice(-20)}catch{}refresh()})
