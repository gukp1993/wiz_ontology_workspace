...
- **新建 worktree 默认同步 main 库（2026-09-21 新增，用户指令）**：为用户创建开发 worktree 时，隔离数据根不再从空库开始，默认把 main 的 SQLite 一并拷贝过去。固定操作：①用 `python3 -m workbench.storage.transfer backup --output <隔离数据根>/data/workbench.sqlite3` 生成 WAL 一致性快照直接落位（CLI 无 restore 子命令，快照即完整库；**禁止直接 cp 正在写入的库文件**）；②复制根密钥 `<main数据根>/keys/wb-root.key` 到 `<隔离数据根>/keys/`（目录 0700、密钥 0600）——不复制根密钥则副本中加密凭据（模型密钥、连接密码）全部不可解密。要点：快照是时点数据，不自动跟随 main；账号口令与 main 相同，用户要求时可在副本内重置（不影响 main）；数据根自此含真实数据与根密钥副本，workbench-tasks 登记必须标注"含真实数据，不可自动丢弃"，集成后自动清理对数据根须经用户确认处置；自动化测试的临时隔离根（WIZ_WORKBENCH_ROOT）不适用本条，仍用空库；用户明确要求空库或合成数据时从其指令。
