...
- **默认并行执行（2026-09-22，用户指令）**：执行任何任务时默认先做并行拆分——在文件归属互斥、接口与资源不冲突、可独立验证的前提下，用尽可能多的子代理并行推进；协调者负责拆分、冲突检查（文件/接口/端口/资源）与串行收口（组合回归、提交、交接报告）。强耦合、同文件或强依赖任务保持串行，不为并行牺牲可验证性；并行不改变既有授权与停止点（验收/集成/合并仍需用户明确指令）。
