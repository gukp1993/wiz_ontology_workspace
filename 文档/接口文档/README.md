...
| 2026-09-21 | **R02 修复：项目校验检查被引用编排的配置有效性**：`/api/project-validate` 对项目引用的编排调用纯配置检查（flows.check_flow，缓存每编排一次，不含连接/LLM/凭据维度）；编排配置错误（如输出未绑定）转为单条阻断项，消息含对象/属性定位、编排标识与原因；warnings 不阻断；未被引用的编排不受影响；不存在的编排维持"不存在"阻断并补定位。发布路径复用同一校验（422，版本零新增），依赖快照重验机制不变。金样新增 2 样例（57/58），旧样例逐字节不变。详见 03 §2.2 | POST /api/project-validate（errors）、/api/project-publish（新增 422 场景） | zcode |
