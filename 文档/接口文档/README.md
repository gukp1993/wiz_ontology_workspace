...
| 日期 | 变更 | 影响接口 | 登记人 |
| --- | --- | --- | --- |
| 2026-09-22 | **04 分册 LLM 提供方新增「思考强度」设置**：§4.1/§4.2/§4.4 增可选字段 `thinking`（`default`\|`off`，缺省 `default`；`off` 时对 `bigmodel.cn` 端点请求体追加 `thinking:{"type":"disabled"}`，其他提供方存值不追加），存储列 `wb_model_configs.thinking`（迁移 20260922_0005）；动机：实测 GLM-5.3-Flash 关闭思考后单批抽取 150–260s→15.2s（本机 2026-09-22 实测，验收方提供） | `/api/llm-providers`、`/api/llm-provider-save`、`/api/llm-provider-test`（临时配置形态） | zcode |
| 2026-09-23 | **自动填写 v2 协议增补（需求《20260922_整表自动填写交互/需求说明_v2.md》）**：①04 §4.6 新增「自动填写」用途配置端点 `GET /api/llm-purpose`、`POST /api/llm-purpose-save`（按账号指定用途模型与思考覆盖 `follow\|off`，复用已存提供方与密钥，不改默认模型；`provider-missing` 不静默回退，`off` 需提供方支持）；②表单契约 `ai` 键新增字段级生成策略 `strategy`（`draft`/`grounded`/`candidate`/`system`，缺省 `grounded`；策略变化须 schemaVersion+1），通用提示词按策略逐字段组织；③assist 响应 meta 新增 `modelDurationMs`（模型耗时）与 `thinkingApplied`（实际思考覆盖），与总耗时分开记录；④04 §6.7 前端行为 v2（侧栏只读展示生效模型、停止等待、实际等待计时、错误保留提示词可重试、字段展示用契约中文 label）；端点与 operations 语义不变 | `/api/llm-purpose`（新增）、`/api/llm-purpose-save`（新增）、`/api/assist-generate`（meta 增字段）、`/api/assist-context`（无字段变化） | zcode |
| 2026-09-22 | **08 分册新增 §14 生成控输出契约冻结（v3/D00，默认关闭未实施）**：环境开关与预算配置、generate 入口 422 错误码五枚（BUDGET_PROFILE_REQUIRED/_MISMATCH/_CONFIG_INVALID/_PLAN_TOO_LARGE/_PLAN_MISMATCH）、schema2 轮询摘要（jobs/coverage/budget/blocking）、Run.usage 可空 token 统计（knownCompletionTokens/unknownUsageCalls，reasoning 不双计）、capabilities 新增 generationBudget；内部结构唯一来源 `workbench/ontology_build/batch_contracts.py` | `/api/build-capabilities`、`/api/build-run`、`/api/build-scope-confirm`、`/api/build-regenerate`、`/api/build-run-resume`（随 D16 接线生效） | zcode |
| 2026-09-22 | **08 分册五项正确性缺陷整改（R1–R5，Codex 审核意见复现确认）**：R1 跨批临时键冲突（批次命名空间 `alignment.namespace_batch` + 引用同步重写 + 模型 key 规范化）；R2 冲突默认纳入（合并降级后未经确认的 include 自动撤销为 defer，issue 码 `AUTO_INCLUDE_REVOKED`；预检/交付端硬门禁，§8.1 新增 `conflictExcluded`/`conflictExcludedItems`）；R3 事实去重身份扩为 snippet+kind+locator+data 语义键（`plan.duplicateOf` 映射、duplicates=实际剔除数）；R4 拆批半失败父批按失败记账（成功半候选保留、重试整批幂等）；R5 批次完成即独立落库（废弃连续前缀游标、落库失败计 failed、调度收尾核对覆盖）；删除死代码 `_batch_wait_timeout`。详见 08 §1.6/§5/§8.1 及变更记录 | `/api/build-deliver-precheck`、`/api/build-deliver`（选定集合口径+新字段）、`/api/build-run`（plan.duplicateOf）、`/api/build-run-resume`（部分失败重跑幂等）、候选 `issues[].code` | zcode |
| 2026-09-22 | **整表自动填写协议冻结（autofill/1，T0）**：`mode=fill` 升级为受限字段操作协议——响应新增 `protocol/status/formId/schemaVersion/schemaDigest/draftFingerprint/sessionId/roundId/operations/questions/unresolved/summary`，operations 仅 set/clear/row.append/row.update/row.remove （契约内点路径＋本地 rowId＋basis 依据核验），同字段冲突拒绝不后项覆盖；请求必带 `protocol:2`，续轮须重取 contextToken 并携 sessionId+answers；会话/问题归属服务端内存绑定（TTL 30min）。表单契约单一来源 `contracts/forms/*.json`（loader `assist_forms.py`，生成器 `assist_forms_gen`→`formContracts.gen.ts`）；旧 suggestions 交互移除，check/explain 保持 §5.2 兼容。详见 04 §6 | POST /api/assist-generate（fill 响应结构重定义）、/api/assist-context（无字段变化） | zcode |
| 2026-09-22 | **assist-context 请求新增可选 `ontologyId` 字段**（本体工作区 id；space=ontology 有效，省略/空=默认 `storage`）：后端按该工作区构建上下文/候选/权威指纹，签入 contextToken 并在 assist-generate 一致性校验（不匹配 409 CONTEXT_STALE）；工作区不可见或属他人一律 404（修复原实现静默回落默认工作区的缺陷）。前端本体区宿主经 `provide('ontology-id')` 透传。浏览器独立验收两轮驱动（DEF-01）。详见 04 §5.1 | POST /api/assist-context（请求字段新增）、/api/assist-generate（令牌校验口径） | zcode |
| 2026-09-22 | **08 分册 V2-10 解析并发（线程池，G25）**：`limits` 新增 `parseConcurrency`（默认 `min(8, CPU 核数)`，环境变量 `WIZ_BUILD_PARSE_CONCURRENCY` 覆盖，正整数/非法回退默认）；§4 `build-scan` 冻结并发模型——单文件粒度池内并行解析、worker 只解析不写库、主线程按物料登记顺序收集并逐个短事务落库（facts 内容与顺序同串行版完全一致）、LLM 兜底与 index 串行、取消 `shutdown(wait=False, cancel_futures=True)`、单文件等待超时标 failed「解析超时」且迟到结果丢弃不落库、worker 异常（含适配器契约外返回值）只影响该文件不杀死 run | `/api/build-capabilities`（limits.parseConcurrency）、`/api/build-scan`（并发语义） | zcode |
...
| 2026-09-21 | **辅助填写接口实现登记（T4）**：`/api/assist-context`、`/api/assist-generate` 接入 `server.py` POST 白名单（97→99 与 05 速查表一致）。契约增量：① 上下文候选截断显式标记（`definitionsTruncated`/`catalogTruncated`/`flowsTruncated` 及条目级 `fieldsTruncated` 等）；② 违规分层口径冻结——整体结构违规 502 `MODEL_BAD_RESPONSE`，单条违规丢弃或转 blocked（详见 04 §5.2）；③ answers ≤8 条（每条 value≤2000/unsure 布尔）；④ generate 时权威上下文重取并与令牌指纹比对（同源校验防基线错位）；⑤ 未配置默认模型 422 `MODEL_NOT_CONFIGURED`，调用超时 504 `MODEL_TIMEOUT`，其余模型失败 502 | POST /api/assist-context、/api/assist-generate；04 §5 | zcode |
| 2026-09-21 | **新增表单辅助填写接口（本体与项目辅助填写正式实现，T0 协议冻结）**：`POST /api/assist-context`（按当前账号可见范围裁剪只读上下文＋HMAC 签名 contextToken，TTL 600s，绑定用户/目标/权威指纹/draft 摘要）与 `POST /api/assist-generate`（真实模型结构化建议；mode=fill/check/explain；建议 state=ready/pending/blocked 由后端按业务规则裁定，白名单外字段键、幻觉引用、越界/超限输出一律拒绝；错误码 409 CONTEXT_STALE / 422 MODEL_NOT_CONFIGURED / 502 MODEL_BAD_RESPONSE / 504 MODEL_TIMEOUT；两接口均不写业务修订、不持全局写锁、trace 不出服务端）。字段白名单唯一来源 `workbench/assist_fields.py`（10 个 targetKind 覆盖 O1～O5、P1～P6；propertySource 按 draft.kind 分派 field/database/redis/flow 子白名单）；前端协议镜像 `frontend/src/assist/types.ts`。上限冻结：intent≤4000、answers≤2000/条、questions≤3、suggestions≤12、issues≤30、draft≤200KB、max_tokens 4000、超时沿用提供方配置。05 速查表 97→99 | POST /api/assist-context、POST /api/assist-generate（新增）；04 新增 §5 | zcode |
