...
|---|---|---|---|---|---|
| A | zcode 子代理 | 修复中（W3） | — | 验收证据见 W0（18 步/21/21/金样） | 三项最小修复 B1/B2/B3 处理中 |
| B | zcode 子代理 | 实施中 | — | — | 依赖三态、绑定失效报错、可复核影响匹配、金样预期条目 |
| C | zcode 子代理 | 已落盘待回收 | — | `tests/test_connection_catalog.py`、`connection_manager.test.mjs` 已建 | 指纹排除显示名、凭据代际、条件写、读取失败三态、幽灵凭据收紧、删除先落盘再提示（已落盘，待其报告） |
| D | zcode 子代理 | 实施中 | — | `checkBaseline.ts`、`project_check_staleness`/`project_publish_receipt` 已建 | 检查代际、概览失败态、发布 key 轮换（已派回修正：签名不含 revision）、比较基线 |
| E | zcode 子代理 | 已落盘待回收 | — | `tests/source_config_retention.test.mjs` 已建 | 换表保留、flow 三态缓存、未知 kind 只读（已落盘，待其报告） |
| L集成 | 协调者 zcode | 实施中 | — | `test_publish_guards.py` **12 步（a–l）**全过（含 stale 链路）；quick 3/3、unit 27/27、`test_project_api_roundtrip` 13 步、`test_save_iteration` 8 步、`test_references`、`test_mapping_descriptions`、金样 97/507 | 金样最终生成（待 B 预期条目）；`npm run build`；隔离实例验收 |
| F独立QA | zcode 子代理 ×2 | 进行中 | — | 新增对抗性测试（发布守卫/目录语义）与前端保护独立复核 | 故障注入结论、范围审计 |
...
**既有失败登记（非本期引入，不掩盖）**：`tests/mapping_forms.test.mjs` 自上一轮改版起持续失败（已在基线 `82923e3`、`1854e5d`、`f2566c6`、`76cf294` 四个提交的独立 worktree 复现，首个失败点分别为第 52/62 行）——该测试覆盖的「新配置 computed/规则入口」已被函数编排取值取代（`PropertySources` 现仅提供数据源/函数编排/登记信息三类卡片，computed 为只读兼容），测试未随该轮 UI 改版同步。本期 P02（映射阅读与编辑）明确延期、不含该 UI 重构，故本轮**不复写**该测试；现行编排绑定路径由 `tests/flow_editor_bind.test.mjs`（通过）覆盖。此项在最终报告中列为「既有失败、范围外」。
