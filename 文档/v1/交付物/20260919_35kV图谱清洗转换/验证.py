...
with tempfile.TemporaryDirectory(prefix='wiz-graph-review-') as tmp:
 ...
 data=(out/'35kV本体_清洗待确认.zip').read_bytes(); digest=hashlib.sha256(data).hexdigest()
 ...
 assert len(loaded['ontology']['extensions']['conversionProvenance']['originalGraph']['@graph'])==73
 result={'schemaValidation':'passed','encodeDecodeRoundtrip':'passed','manifestAndHash':'passed','isolatedImportPreview':'passed','isolatedImportAndReadback':'passed','originalGraphPreserved':73,'expectedPublishBlockers':errors,'realWorkbenchModified':False,'browserTested':False}
