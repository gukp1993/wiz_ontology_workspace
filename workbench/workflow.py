...
def _business_rule_errors(state, classes):
    """业务规则校验：名称/业务定义为非空文本（20260920 精简；content/output 选填，
    content 提供时必须为文本——A01 20260921）、标识唯一、引用存在、组合唯一。"""
    ...
    for index, rule in enumerate(rules, 1):
        ...
        # 20260920 字段精简：名称/业务定义必填文本（经 _text_field_errors 连类型一起校验）；
        # 历史规则内容/输出选填（output 只保留不校验）。
    ...
    for kind in ('functions','actions','interfaces'):
        for n in w.get(kind,[]):
            ...
            if kind=='actions' and is_action_v2(n):
                ...
                # 对象/数组/数值/布尔报单条错误（口径与 businessRules 的 content 一致，含名称/标识/
                # 当前类型与修复建议），不隐式转字符串、不改键、不清洗。
                effect=n.get('effect')
